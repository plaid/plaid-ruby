module Plaidio
  ...
  class Customer
    ...
    def get(path,access_token,options={})
      url = BASE_URL + path
      @response = RestClient.get url, :client_id => self.instance_variable_get(:'@customer_id'), :secret => self.instance_variable_get(:'@secret'), :access_token => access_token
      return @response
    end
    ...
    def delete(path,access_token,options={})
      url = BASE_URL + path
      @response = RestClient.delete url, :client_id => self.instance_variable_get(:'@customer_id'), :secret => self.instance_variable_get(:'@secret'), :access_token => access_token
      return @response
    end
  end
end