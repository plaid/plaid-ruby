  ...
  class UpdateEntityScreeningRequestSearchTerms
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::UpdateEntityScreeningRequestSearchTerms` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::UpdateEntityScreeningRequestSearchTerms`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@legal_name.nil? && @legal_name.to_s.length < 1
    ...
    def valid?
      ...
      return false if !@legal_name.nil? && @legal_name.to_s.length < 1
    ...
    def hash
      [entity_watchlist_program_id, legal_name, document_number, email_address, country, phone_number, url, client_id, secret].hash
    end
