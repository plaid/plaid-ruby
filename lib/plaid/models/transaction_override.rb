  ...
  class TransactionOverride
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::TransactionOverride` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::TransactionOverride`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @date_transacted.nil?
        invalid_properties.push('invalid value for "date_transacted", date_transacted cannot be nil.')
      end

      if @date_posted.nil?
        invalid_properties.push('invalid value for "date_posted", date_posted cannot be nil.')
      end
    ...
    def hash
      [date_transacted, date_posted, amount, description, currency].hash
    end
