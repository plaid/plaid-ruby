  ...
  class TransferRepaymentReturnListRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::TransferRepaymentReturnListRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::TransferRepaymentReturnListRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'repayment_id')
        self.repayment_id = attributes[:'repayment_id']
      end
    ...
    def list_invalid_properties
      ...
      if @repayment_id.nil?
        invalid_properties.push('invalid value for "repayment_id", repayment_id cannot be nil.')
      end
    ...
    def hash
      [client_id, secret, repayment_id, count, offset].hash
    end
