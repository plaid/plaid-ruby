  ...
  class DepositSwitchAltCreateRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::DepositSwitchAltCreateRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::DepositSwitchAltCreateRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'target_user')
        self.target_user = attributes[:'target_user']
      end
    ...
    def list_invalid_properties
      ...
      if @target_user.nil?
        invalid_properties.push('invalid value for "target_user", target_user cannot be nil.')
      end
    ...
    def hash
      [client_id, secret, target_account, target_user, options, country_code].hash
    end
