  ...
  class TransferIntentGetResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::TransferIntentGetResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::TransferIntentGetResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      authorization_decision_validator = EnumAttributeValidator.new('String', ["APPROVED", "PERMITTED", "DECLINED"])
      return false unless authorization_decision_validator.valid?(@authorization_decision)
    ...
    def authorization_decision=(authorization_decision)
      validator = EnumAttributeValidator.new('String', ["APPROVED", "PERMITTED", "DECLINED"])
      unless validator.valid?(authorization_decision)
        fail ArgumentError, "invalid value for \"authorization_decision\", must be one of #{validator.allowable_values}."
      end
      @authorization_decision = authorization_decision
    end
    ...
    def hash
      [id, created, status, transfer_id, failure_reason, authorization_decision, authorization_decision_rationale, account_id, origination_account_id, amount, mode, ach_class, user, description, metadata].hash
    end
