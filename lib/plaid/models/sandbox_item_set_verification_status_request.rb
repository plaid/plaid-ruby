  ...
  class SandboxItemSetVerificationStatusRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::SandboxItemSetVerificationStatusRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::SandboxItemSetVerificationStatusRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def verification_status=(verification_status)
      validator = EnumAttributeValidator.new('String', ["automatically_verified", "verification_expired"])
    ...
    def hash
      [client_id, secret, access_token, account_id, verification_status].hash
    end
