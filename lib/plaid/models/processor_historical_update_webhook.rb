  ...
  class ProcessorHistoricalUpdateWebhook
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::ProcessorHistoricalUpdateWebhook` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::ProcessorHistoricalUpdateWebhook`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'new_transactions')
        self.new_transactions = attributes[:'new_transactions']
      end
    ...
    def list_invalid_properties
      ...
      if @new_transactions.nil?
        invalid_properties.push('invalid value for "new_transactions", new_transactions cannot be nil.')
      end
    ...
    def hash
      [webhook_type, webhook_code, error, new_transactions, account_id, environment].hash
    end
