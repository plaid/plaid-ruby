  ...
  class BankInitiatedReturnRisk
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::BankInitiatedReturnRisk` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::BankInitiatedReturnRisk`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @risk_tier > 8
        invalid_properties.push('invalid value for "risk_tier", must be smaller than or equal to 8.')
      end
    ...
    def risk_tier=(risk_tier)
      ...
      if risk_tier > 8
        fail ArgumentError, 'invalid value for "risk_tier", must be smaller than or equal to 8.'
      end
