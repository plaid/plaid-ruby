  ...
  class PaymentInitiationRecipientListResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::PaymentInitiationRecipientListResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::PaymentInitiationRecipientListResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@next_cursor.nil? && @next_cursor.to_s.length > 256
        invalid_properties.push('invalid value for "next_cursor", the character length must be smaller than or equal to 256.')
      end
    ...
    def valid?
      ...
      return false if !@next_cursor.nil? && @next_cursor.to_s.length > 256
      true
    end
    ...
    def next_cursor=(next_cursor)
      if !next_cursor.nil? && next_cursor.to_s.length > 256
        fail ArgumentError, 'invalid value for "next_cursor", the character length must be smaller than or equal to 256.'
      end
    ...
    def hash
      [recipients, request_id, next_cursor].hash
    end
