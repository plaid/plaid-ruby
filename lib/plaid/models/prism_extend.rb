  ...
  class PrismExtend
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::PrismExtend` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::PrismExtend`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @model_version.nil?
        invalid_properties.push('invalid value for "model_version", model_version cannot be nil.')
      end
    ...
    def valid?
      return false if @model_version.nil?
      true
    end
    ...
    def hash
      [model_version, score, reason_codes, metadata, error_reason].hash
    end
