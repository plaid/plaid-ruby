...
module Plaid
  ...
  class CraCreditProfileLendScore
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::CraCreditProfileLendScore` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::CraCreditProfileLendScore`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @reason_codes.nil?
        invalid_properties.push('invalid value for "reason_codes", reason_codes cannot be nil.')
      end
    ...
    def valid?
      return false if @reason_codes.nil?
    ...
    def hash
      [score, reason_codes, variant, error_reason].hash
    end
