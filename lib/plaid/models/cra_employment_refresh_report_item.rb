...
module Plaid
  ...
  class CraEmploymentRefreshReportItem
    ...
    def self.openapi_types
      {
        :'accounts' => :'Array<CraEmploymentRefreshReportAccount>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::CraEmploymentRefreshReportItem` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::CraEmploymentRefreshReportItem`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @last_update_time.nil?
        invalid_properties.push('invalid value for "last_update_time", last_update_time cannot be nil.')
      end
    ...
    def hash
      [accounts, institution_name, institution_id, item_id, last_update_time].hash
    end
