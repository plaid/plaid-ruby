...
module Plaid
  ...
  class ExtendedRecipientMetadata
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::ExtendedRecipientMetadata` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::ExtendedRecipientMetadata`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'joined_date')
        self.joined_date = attributes[:'joined_date']
      end
    ...
    def list_invalid_properties
      ...
      if @joined_date.nil?
        invalid_properties.push('invalid value for "joined_date", joined_date cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @joined_date.nil?
    ...
    def hash
      [recipient_id, client_name, logo_uri, third_party_legal_name, category, joined_date, connection_count].hash
    end
