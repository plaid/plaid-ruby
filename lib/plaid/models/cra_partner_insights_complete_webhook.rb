...
module Plaid
  # Fired when a partner insights report has finished generating or failed to generate.
  class CraPartnerInsightsCompleteWebhook
    # `CRA_INSIGHTS`
    ...
    def self.openapi_types
      {
        ...
        :'webhook_code' => :'String',
        :'user_id' => :'String',
        :'result' => :'CraPartnerInsightsCompleteResult',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::CraPartnerInsightsCompleteWebhook` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::CraPartnerInsightsCompleteWebhook`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [webhook_type, webhook_code, user_id, result, environment].hash
    end
