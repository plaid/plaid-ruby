...
module Plaid
  ...
  class ProcessorSignalDecisionReportRequest
    ...
    attr_accessor :days_funds_on_hold

    attr_accessor :decision_outcome
    ...
    attr_accessor :amount_instantly_available
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::ProcessorSignalDecisionReportRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::ProcessorSignalDecisionReportRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] days_funds_on_hold Value to be assigned
    def days_funds_on_hold=(days_funds_on_hold)
      if !days_funds_on_hold.nil? && days_funds_on_hold < 0
        fail ArgumentError, 'invalid value for "days_funds_on_hold", must be greater than or equal to 0.'
      end
    ...
    def hash
      [client_id, secret, processor_token, client_transaction_id, initiated, days_funds_on_hold, decision_outcome, payment_method, amount_instantly_available].hash
    end
