...
module Plaid
  ...
  class PSLFStatus
    ...
    attr_accessor :estimated_eligibility_date
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::PSLFStatus` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::PSLFStatus`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'estimated_eligibility_date')
    ...
    def ==(o)
      ...
      self.class == o.class &&
          estimated_eligibility_date == o.estimated_eligibility_date &&
    ...
    def hash
      [estimated_eligibility_date, payments_made, payments_remaining].hash
    end
