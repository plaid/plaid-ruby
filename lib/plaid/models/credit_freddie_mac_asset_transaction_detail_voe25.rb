  ...
  class CreditFreddieMacAssetTransactionDetailVOE25
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::CreditFreddieMacAssetTransactionDetailVOE25` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::CreditFreddieMacAssetTransactionDetailVOE25`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [asset_transaction_unique_identifier, asset_transaction_date, asset_transaction_post_date, asset_transaction_type, asset_transaction_paid_by_name, asset_transaction_paid_to_name, asset_transaction_type_additional_description, asset_transaction_category_type, financial_institution_transaction_identifier].hash
    end
