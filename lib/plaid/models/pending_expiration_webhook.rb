  ...
  class PendingExpirationWebhook
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::PendingExpirationWebhook` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::PendingExpirationWebhook`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @consent_expiration_time.nil?
        invalid_properties.push('invalid value for "consent_expiration_time", consent_expiration_time cannot be nil.')
      end
    ...
    def hash
      [webhook_type, webhook_code, item_id, consent_expiration_time, environment].hash
    end
