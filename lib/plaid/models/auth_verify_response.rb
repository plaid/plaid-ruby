  ...
  class AuthVerifyResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::AuthVerifyResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::AuthVerifyResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @verification_insights.nil?
        invalid_properties.push('invalid value for "verification_insights", verification_insights cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @verification_insights.nil?
      true
    end
    ...
    def hash
      [request_id, item_id, verification_status, verification_insights].hash
    end
