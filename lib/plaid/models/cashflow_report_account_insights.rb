...
module Plaid
  ...
  class CashflowReportAccountInsights
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::CashflowReportAccountInsights` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::CashflowReportAccountInsights`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @monthly_summaries.nil?
        invalid_properties.push('invalid value for "monthly_summaries", monthly_summaries cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @monthly_summaries.nil?
      true
    end
    ...
    def hash
      [historical_balances, monthly_summaries].hash
    end
