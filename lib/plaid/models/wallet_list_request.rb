  ...
  class WalletListRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::WalletListRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::WalletListRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@cursor.nil? && @cursor.to_s.length > 1024
        invalid_properties.push('invalid value for "cursor", the character length must be smaller than or equal to 1024.')
      end
    ...
    def valid?
      return false if !@cursor.nil? && @cursor.to_s.length > 1024
    ...
    def cursor=(cursor)
      if !cursor.nil? && cursor.to_s.length > 1024
        fail ArgumentError, 'invalid value for "cursor", the character length must be smaller than or equal to 1024.'
      end
    ...
    def hash
      [client_id, secret, iso_currency_code, cursor, count].hash
    end
