...
module Plaid
  ...
  class PendingDisconnectWebhook
    # `ITEM`
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::PendingDisconnectWebhook` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::PendingDisconnectWebhook`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'disconnect_time')
        self.disconnect_time = attributes[:'disconnect_time']
      end
    ...
    def list_invalid_properties
      ...
      if @disconnect_time.nil?
        invalid_properties.push('invalid value for "disconnect_time", disconnect_time cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @disconnect_time.nil?
    ...
    def hash
      [webhook_type, webhook_code, item_id, user_id, reason, disconnect_time, environment].hash
    end
