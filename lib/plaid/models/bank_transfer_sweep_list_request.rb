  ...
  class BankTransferSweepListRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Plaid::BankTransferSweepListRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Plaid::BankTransferSweepListRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'start_id')
        self.start_id = attributes[:'start_id']
      end
    ...
    def list_invalid_properties
      ...
      if !@start_id.nil? && @start_id < 0
        invalid_properties.push('invalid value for "start_id", must be greater than or equal to 0.')
      end
    ...
    def valid?
      return false if !@start_id.nil? && @start_id < 0
    ...
    # @param [Object] start_id Value to be assigned
    def start_id=(start_id)
      if !start_id.nil? && start_id < 0
        fail ArgumentError, 'invalid value for "start_id", must be greater than or equal to 0.'
      end
    ...
    def hash
      [client_id, secret, origination_account_id, start_id, start_time, end_time, count].hash
    end
