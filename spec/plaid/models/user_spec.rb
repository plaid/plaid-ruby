describe Plaid::User do
  ...
  let(:auth_user)    { client.add_user('auth',    'plaid_test', 'plaid_good', 'wells') }
  let(:connect_user) { client.add_user('connect', 'plaid_test', 'plaid_good', 'wells') }
  ...
  describe '#mfa_authentication' do
    let(:user) { client.add_user('connect', 'plaid_test', 'plaid_good', 'bofa') }
    ...
    context 'enters incorrect credentials for MFA auth' do
      ...
      let(:mfa_bad)  { mfa_user; client.add_user('connect', 'plaid_test', 'plaid_good', 'bofa') }
    ...
    context 'requests list of MFA credentials' do
      let(:new_mfa_user) { client.add_user('auth', 'plaid_test', 'plaid_good', 'chase', nil, '{"list":true}') }
    ...
    context 'selects MFA method and returns successful response' do
      let(:user) { client.add_user('auth', 'plaid_test', 'plaid_good', 'chase', nil, '{"list":true}') }
  ...
  describe '#get_balance' do
    ...
    let(:user) { client.add_user('info', 'plaid_test', 'plaid_good', 'wells') }
