describe Plaid::Institution do
  context 'when a single institution is found' do
    let(:institution) { test_client.institution('5301a93ac140de84910000e0') }
  ...
  context 'when all institutions are found' do
    let(:institution) { test_client.institution }
    it { expect(institution).to be_kind_of(Array) }
  end

  context 'when institution is not found' do
    it { expect { test_client.institution('dumb_bank') }.to raise_error(Plaid::NotFound, 'unable to find institution') }
  end
end
